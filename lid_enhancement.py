    ...
    def seq_forward(self, X):
        ...
        for b_i in tqdm(range(len(X))):
            ...
            if self.lid_silero_enable:
                ...
                pred_keys, pred_probs = [pred_lang[i][0][:2] for i in range(self.lid_return_n)], [pred_lang[i][1] for i in range(self.lid_return_n)]
        ...
        pred = [max(batch_pred, key=batch_pred.get) for batch_pred in lid_result]
        pred = [self.code2label for lid in pred]
        return pred
